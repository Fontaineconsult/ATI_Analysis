...
def find_year_success_evidence_by_academic_year(academic_year_name: str, campus_abbreviation: str = None) -> list:
    """
    ...
    :param campus_abbreviation: Optional campus abbreviation to filter by (e.g., 'sfsu', 'ssu', 'csueb').
    ...
    try:
        ...
        for yse in year_success_evidences:
            # If campus filter is specified, check the campus relationship
            if campus_abbreviation:
                campus_nodes = yse.campus.all()
                if not any(c.abbreviation == campus_abbreviation for c in campus_nodes):
                    continue

            success_indicator = yse.tracks_success_indicator.single()

            # Get the campus for this YSE
            campus_nodes = yse.campus.all()
            campus_abbrev = campus_nodes[0].abbreviation if campus_nodes else None

            results.append({
                "year_identifier": yse.year_identifier,
                "success_indicator": success_indicator.success_indicator,
                "campus_abbreviation": campus_abbrev
            })
...
def get_evidence_trends(past_year, current_year, working_group=None, campus_abbreviation=None):
    ...
    :param campus_abbreviation: Optional campus abbreviation to filter by (e.g., 'sfsu').
    ...
    try:
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # Construct the absolute path to the query file
        query_file_path = os.path.join(current_dir, '../../batch/trends_campus.cypher')
        # Normalize the path
        query_file_path = os.path.normpath(query_file_path)
