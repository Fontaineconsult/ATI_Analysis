...
def fetch_evidence_for_working_group(working_group, academic_year, campus_abbreviation=None):
    ...
    OPTIONAL MATCH (evidence)-[:evidence_at_campus]->(evidenceCampus:Campus)

    // Optional campus filter
    WITH wg, goal, indicator, evidence, evidenceCampus
      WHERE $campus_abbreviation IS NULL OR evidenceCampus.abbreviation = $campus_abbreviation
    ...
    WITH wg, goal, indicator, evidence, evidenceCampus, evidenceNotes,
         collect(DISTINCT {
    ...
    WITH wg, goal, indicator, evidence, evidenceCampus, evidenceNotes, evidenceMessages,
         collect(DISTINCT {
    ...
    WITH wg, goal, indicator, evidence, evidenceCampus, evidenceNotes, evidenceMessages, evidenceMetrics, statusLevel,
    ...
    WITH wg, goal, indicator, evidence, evidenceCampus, evidenceNotes, evidenceMessages, evidenceMetrics, statusLevel, persons, adminReviewers,
    ...
    WITH wg, goal, indicator, evidence, evidenceCampus, evidenceNotes, evidenceMessages, evidenceMetrics, statusLevel, adminReviewers, adminReviewNotes, persons, plans, plansWithNotes, evidenceType,
         collect(DISTINCT {
           document: doc,
    ...
    // strength rides the is_evidence_for rel itself (IsEvidenceForRel) — re-derived
    // here via a pattern comprehension so it needn't thread through every WITH above.
    WITH wg, goal, indicator, evidence, evidenceCampus, evidenceNotes, evidenceMessages, evidenceMetrics, statusLevel, adminReviewers, adminReviewNotes, persons, plans, plansWithNotes,
         {
    ...
    // Create a map for each evidence with its data, including plans and campus
    WITH wg, goal, indicator, statusLevel, adminReviewers, adminReviewNotes, persons, plans, plansWithNotes, evidenceTypes, evidence, evidenceCampus, evidenceNotes, evidenceMessages, evidenceMetrics,
         {
           evidence: evidence,
           campus: evidenceCampus,
    ...
    // (and at the selected campus, when one is given). The remediates /
    // remediates_interface / uses_tool rel-types only originate on
    // Process/Project/Procedure/Service, so the impl node needs no label filter.
    ...
    // parent asset of a touched tool (Tool-tool_of_asset->Asset).
         ...
         apoc.coll.toSet(
           [ (indicator)<-[:tracks]-(yse:YearSuccessEvidence)<-[:is_evidence_for]-(impl)-[:remediates]->(asset:Asset)
               WHERE (yse)-[:evidence_in_year]->(:AcademicYear {name: $academic_year})
                 AND ($campus_abbreviation IS NULL OR (yse)-[:evidence_at_campus]->(:Campus {abbreviation: $campus_abbreviation}))
