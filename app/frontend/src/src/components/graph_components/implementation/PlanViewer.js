...
function PlanViewer({ plans, onSubmit, yearSuccessEvidence, createdBy }) {
    ...
    const handleFormSubmit = async (index, planData, isNew) => {
        try {
            if (isNew) {
                await createPlan({ furthered_yse_identifier:yearSuccessEvidence, ...planData, academic_year_name: currentAcademicYear });
            } else {
                await updatePlan({ furthered_yse_identifier:yearSuccessEvidence, ...planData, academic_year_name: currentAcademicYear });
            }
