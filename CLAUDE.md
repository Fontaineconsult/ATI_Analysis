...
`/stakeholder-interview` — preps a stakeholder interview: recons the graph for the campus/topic and generates a guide in `app/database/ontology/interviews/` built around **the maturity bar**. Each target indicator's companion guide (`established_example` / `examples_of_evidence`, falling back to the generic Status Level rubric when none is authored) is decomposed into its elements — Position / Budget / Procedures / Output — and each element gets what the graph holds today, the question that settles it, and the artifact that would evidence it. Targets are ranked by distance to Established, not by weakness. The goal is to collect enough to earn Established while refusing to claim it when the evidence doesn't match the bar. Also carries follow-ups from prior minutes, open Queries to settle live, and the standing protocol + transcript-hygiene habits. The transcript feeds `/ontology-ingest` (which reports coverage element by element), then `/maturity-status-reviewer` grades it. Triggered by "prep an interview with X", "I'm meeting X about Y".
