...
`/stakeholder-interview` — preps a stakeholder interview: recons the graph for the campus/topic and generates a one-page guide in `app/database/ontology/interviews/` (follow-ups from prior minutes, open Queries to settle live, SI coverage gaps to probe, artifact wishlist) wrapped around the standing interview protocol + transcript-hygiene habits. The resulting transcript feeds `/ontology-ingest`, which reports coverage against the guide. Triggered by "prep an interview with X", "I'm meeting X about Y".
